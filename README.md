...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`mime`](https://crates.io/crates/mime).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `mime` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Strongly typed MIME / Media Types
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:mime-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `mime` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
