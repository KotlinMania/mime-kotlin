# Claude Code Project Instructions — mime-kotlin

## Project Overview

This is **mime-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`mime`](https://crates.io/crates/mime).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/mime/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/mime>
